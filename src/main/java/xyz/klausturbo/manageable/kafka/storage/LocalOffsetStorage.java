 ...
 * 用于存储成功消费的 offset .
 * <p>
 * 考虑到 offset消息 的消费与最终的 offset 的提交之间存在系统崩溃的可能，采用每次消费成功之后，本地持久化本次消费的消息的offset， 用于系统恢复之后offset的再提交以及防止消息的重复消费.
 * <p>
 * 例如：消费者单次拉 500 条消息，并且手动提交设置为 500，当消费成功 200 条之后，系统出现崩溃，此时的前200 条消息已经成功消费，但offset还未提交到kafka,
 * 当再次恢复之后，前200条消息可能重复消费（排除业务逻辑的幂等）.
